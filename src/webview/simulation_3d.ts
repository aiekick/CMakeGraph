// ------------------------------------------------------------
// 3D Force-directed simulation engine.
// Same structure as Simulation2D but with z-component in all forces.
// ------------------------------------------------------------

import type { GraphEdge, GraphNode, ISimulation, LayoutNode, SimVars } from './types';

export class Simulation3D implements ISimulation {
    private m_nodesCount = 0;
    private m_forcesX = new Float64Array(0);
    private m_forcesY = new Float64Array(0);
    private m_forcesZ = new Float64Array(0);
    ...
    private computeAdjacency(aNodes: LayoutNode[], aEdges: GraphEdge[]): void {
        ...
        this.m_forcesZ = new Float64Array(this.m_nodesCount);
    ...
    private attractLinks(aNodes: LayoutNode[], aEdges: GraphEdge[], aVars: SimVars, aIsFiltered: (n: GraphNode) => boolean): void {
        for (const edge of aEdges) {
            ...
            const dz = aNodes[ti].z - aNodes[fi].z;
            ...
            this.m_forcesZ[fi] += fz;
            this.m_forcesX[ti] -= fx;
            this.m_forcesY[ti] -= fy;
            this.m_forcesZ[ti] -= fz;
        }
    }
    ...
    private dynamicBarycenter(aNodes: LayoutNode[], aVars: SimVars): void {
        ...
        for (let i = 0; i < this.m_nodesCount; i++) {
            ...
            this.m_forcesZ[i] -= ln.z * aVars.gravity * 0.1;
        }
    }
    ...
    private applyForces(
        ...
        for (let i = 0; i < this.m_nodesCount; i++) {
            ...
            const az = this.m_forcesZ[i] / m;

            ln.vx = (ln.vx + ax) * aVars.damping;
            ln.vy = (ln.vy + ay) * aVars.damping;
            ln.vz = (ln.vz + az) * aVars.damping;

            const speed = Math.sqrt(ln.vx * ln.vx + ln.vy * ln.vy + ln.vz * ln.vz);
            if (speed > max_speed) {
            ...
            ln.y += ln.vy;
            ln.z += ln.vz;
            totalEnergy += Math.abs(ln.vx) + Math.abs(ln.vy) + Math.abs(ln.vz);
        }
